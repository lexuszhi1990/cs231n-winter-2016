...
def compute_saliency_maps(X, y, model):
  ...
  N,_,H,W = np.shape(dX)
  saliency = np.max(abs(dX), axis=1).reshape(N, H, W)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return saliency
...
def make_fooling_image(X, target_y, model):
  ...
  iters = 0
  while 1:
    scores, cache = model.forward(X_fooling, mode=mode)
    ...
    dX, grads = model.backward(scores, cache)
    '''
    mean1 = np.mean(X_fooling, axis = 3)
    mean2 = np.mean(mean1, axis = 2)
    print mean2
    mean1 = np.mean(dX, axis = 3)
    mean2 = np.mean(mean1, axis = 2)
    print mean2
    '''
    X_fooling += (100 * dX)
